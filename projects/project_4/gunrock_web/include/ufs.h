...
/*
    ...
    - inode:
        - type = UFS_DIRECTORY
        - int size = at least 64 bytes (.. and .) - each directory entry is 32 bytes
        - unsigned int direct[DIRECT_PTRS] = data block addresses (holds the addresses of the directory entries)
*/
...
{
    int inode_bitmap_addr; // block address (in blocks) // node_bitmap_addr tells me the first block of the inode bitmap
