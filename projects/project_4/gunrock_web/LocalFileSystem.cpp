...
static int findFirstFreeBit(unsigned char *bitmap, size_t bitmapBytes,
                            size_t totalBits) {
...
static int allocateInode(LocalFileSystem *const fs, super_t *super) {
  ...
  int freeInodeIndex =
      findFirstFreeBit(inodeBitmap, inodeBitmapBytes, super->num_inodes);
  if (freeInodeIndex < 0) {
    delete[] inodeBitmap;
...
static int deallocateInode(LocalFileSystem *const fs, super_t *super,
                           int inodeToFree) {
  // Process Inode bitmap
  size_t inodeBitmapBytes = static_cast<int>(ceil(super->num_inodes / 8.0));
  unsigned char *inodeBitmap = new unsigned char[inodeBitmapBytes];
...
static int deallocateDataBlock(LocalFileSystem *const fs, super_t *const super,
                               int dataBlock) {
  // Process Data bitmap
...
static int allocateDataBlock(LocalFileSystem *const fs, super_t *const super) {
  ...
  int freeDataBlockIndex =
      findFirstFreeBit(dataBitmap, dataBitmapBytes, super->num_data);
...
static int validateInodeNumber(int parentInodeNumber, int amountOfNodes) {
  return (parentInodeNumber < 0 || parentInodeNumber >= amountOfNodes)
             ? -EINVALIDINODE
             : 0;
}

static int validateCreateParameters(LocalFileSystem *const fs,
                                    super_t *const super, int parentInodeNumber,
                                    int type, string name) {
  // Check if parent inode number is valid
  int parentInodeValidation =
      validateInodeNumber(parentInodeNumber, super->num_inodes);
...
static int writeData(LocalFileSystem *const fs, super_t *const super,
                     inode_t &inode, char *data, size_t dataSize,
                     size_t oldSize) {
  size_t blocksNeeded =
      std::ceil(static_cast<double>(dataSize) / UFS_BLOCK_SIZE);
  size_t allocatedBlocks =
      std::ceil(static_cast<double>(oldSize) / UFS_BLOCK_SIZE);
  ...
  if (blocksNeeded > allocatedBlocks) {
    int actualBlocksAllocated =
        allocatedBlocks; // keep track of allocated blocks (needed if allocating
                         // fails)
  ...
  for (size_t i = 0; i < blocksNeeded; i++) {
    ...
    size_t bytesToCopy = min(static_cast<size_t>(UFS_BLOCK_SIZE),
                             static_cast<size_t>(dataSize - bytesWritten));
...
static vector<dir_ent_t> readDirectoryEntries(LocalFileSystem *const fs,
                                              const vector<inode_t> &inodes,
                                              int dirInodeNumber) {
...
static int writeDirectoryEntries(LocalFileSystem *const fs,
                                 super_t *const super, vector<inode_t> &inodes,
                                 int dirInodeNumber,
                                 const vector<dir_ent_t> &dirEntries) {
  inode_t &dirInode = inodes.at(dirInodeNumber);
  if (dirInode.type != UFS_DIRECTORY) {
    return -EINVALIDTYPE;
  }
  ...
  for (size_t j = 0; j < dirEntries.size(); j++) {
    memcpy(newDirBuffer + (j * sizeof(dir_ent_t)), &dirEntries.at(j),
           sizeof(dir_ent_t));
  }
  ...
  int bytesWritten =
      writeData(fs, super, dirInode, newDirBuffer, newDirSize, oldDirSize);
  delete[] newDirBuffer;
  return bytesWritten;
}

static int addDirectoryEntry(LocalFileSystem *const fs, super_t *const super,
                             vector<inode_t> &inodes, int parentInodeNumber,
                             int newInodeNum, string name) {
  vector<dir_ent_t> dirEntries =
      readDirectoryEntries(fs, inodes, parentInodeNumber);
  ...
  return writeDirectoryEntries(fs, super, inodes, parentInodeNumber,
                               dirEntries);
}

static int removeDirectoryEntry(LocalFileSystem *const fs, super_t *const super,
                                vector<inode_t> &inodes, int parentInodeNumber,
                                string name) {
  vector<dir_ent_t> dirEntries =
      readDirectoryEntries(fs, inodes, parentInodeNumber);
...
void LocalFileSystem::readInodeBitmap(super_t *super,
  ...
  memcpy(inodeBitmap, buffer,
         static_cast<int>(std::ceil(super->num_inodes / 8.0)));
}
...
void LocalFileSystem::readDataBitmap(super_t *super,
  ...
  memcpy(dataBitmap, buffer,
         static_cast<int>(std::ceil(super->num_data / 8.0)));
}
...
void LocalFileSystem::readInodeRegion(super_t *super, inode_t *inodes) {
  ...
  for (size_t i = 0; i < amountOfBlocks; i++) {
    ...
    size_t inodes_to_copy =
        min(inodes_per_block, num_inodes - (i * inodes_per_block));
    if (inodes_to_copy <= 0) {
      break;
    }
    // Copy inodes from the buffer to the inodes array
    memcpy(&inodes[i * inodes_per_block], buffer,
           inodes_to_copy * sizeof(inode_t));
  }
}
...
void LocalFileSystem::writeInodeRegion(super_t *super, inode_t *inodes) {
  ...
  for (size_t i = 0; i < amountOfBlocks; i++) {
    size_t inodes_to_copy =
        min(inodes_per_block,
            num_inodes -
                (i * inodes_per_block)); // how many inodes to copy from array
    if (inodes_to_copy <= 0) {
      break;
    }
    ...
    memcpy(buffer, &inodes[i * inodes_per_block],
           inodes_to_copy *
               sizeof(inode_t)); // copy inodes from inodes array to buffer
...
int LocalFileSystem::lookup(int parentInodeNumber, string name) {
  ...
  if (this->read(parentInodeNumber, buffer, dirSize) !=
      static_cast<int>(dirSize)) {
    delete[] buffer;
    return -EINVALIDINODE;
  }
...
int LocalFileSystem::read(int inodeNumber, void *buffer, int size) {
  ...
  size_t blocksNeeded =
      std::ceil(static_cast<double>(inode.size) / UFS_BLOCK_SIZE);
  size_t bytesToRead =
      min(static_cast<size_t>(size), static_cast<size_t>(inode.size));
  size_t bytesRead = 0;
  ...
  for (size_t i = 0; i < blocksNeeded; i++) {
    ...
    size_t destOffset =
        i *
        UFS_BLOCK_SIZE; // Offset in destination buffer to start writing from
    size_t bytesToCopy = min(static_cast<size_t>(UFS_BLOCK_SIZE),
...
int LocalFileSystem::create(int parentInodeNumber, int type, string name) {
  ...
  int validationResult =
      validateCreateParameters(this, &super, parentInodeNumber, type, name);
  ...
  if (type == UFS_REGULAR_FILE) {
    ...
    int bytesWritten = writeData(this, &super, inodes[newInodeNum],
                                 (char *)entries, newDirSize, 0);
    delete[] entries;
  ...
  int bytesWritten = addDirectoryEntry(this, &super, inodes, parentInodeNumber,
                                       newInodeNum, name);
  if (bytesWritten < 0) {
    this->disk->rollback();
    return bytesWritten;
...
int LocalFileSystem::write(int inodeNumber, const void *buffer, int size) {
  ...
  int bytesWritten =
      writeData(this, &super, inode, (char *)buffer, size, oldSize);
  if (bytesWritten < 0) {
    this->disk->rollback();
    return bytesWritten;
  }
...
int LocalFileSystem::unlink(int parentInodeNumber, std::string name) {
  ...
  if (inodes.at(inodeToDelete).type == UFS_DIRECTORY) {
    ...
    if (inodes.at(inodeToDelete).size >
        static_cast<int>(sizeof(dir_ent_t) * 2)) {
      this->disk->rollback();
      return -EDIRNOTEMPTY;
    }

    // Remove . and .. entries
    if ((ret = removeDirectoryEntry(this, &super, inodes, inodeToDelete, ".")) <
        0) {
      this->disk->rollback();
      return ret;
    }
  ...
  if ((ret = removeDirectoryEntry(this, &super, inodes, parentInodeNumber,
                                  name)) < 0) {
    this->disk->rollback();
    return ret;
  }
  ...
  if (inodes.at(inodeToDelete).size > 0) {
    for (int i = 0; i < DIRECT_PTRS; i++) {
      if (inodes.at(inodeToDelete).direct[i] != UINT_MAX) {
        if (deallocateDataBlock(this, &super,
                                inodes.at(inodeToDelete).direct[i]) != 0) {
          this->disk->rollback();
