...
HttpService *find_service(HTTPRequest *request) {
  ...
  for (unsigned int idx = 0; idx < services.size(); idx++) {
    if (path.find(services[idx]->pathPrefix()) == 0) {
      return services[idx];
    }
  }
...
void *consumer(void *arg) {
  while (true) {
    ...
    if (dthread_mutex_lock(&threadLock) != 0) {
      cerr << "Error occurred when trying to acquire lock in worker thread"
           << endl;
      continue;
    }

    while (buffer.empty()) {
      // if buffer is empty, wait until filled (release lock to allow other
      // threads to continue)
...
int main(int argc, char *argv[]) {
  ...
  while (true) {
    ...
    MySocket *client =
        server->accept(); // returns a unique client socket object (represents
